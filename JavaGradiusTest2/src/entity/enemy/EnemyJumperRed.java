 ...
 * @author admin
 */
public class EnemyJumperRed extends Entity {
    ...
    public EnemyJumperRed(View view, Ship ship, int x, int y, int powerUpId) {
        super(view);
        this.ship = ship;
        this.x = x;
        this.baseY = 148;
    ...
    public void update() {
        ...
        x += vx;
        y = baseY - Math.abs(60 * Math.sin((z + Time.frameCount) * 0.15));
        
        checkCollision();

        if (x < 130 && Math.abs(y - baseY) < 8 && jumpForward == 0) {
            vx = 3;
            jumpForward = 1;
        }        
        else if (x > 140 && Math.abs(y - baseY) < 8 && jumpForward == 1) {
            vx = -3;
        }       
