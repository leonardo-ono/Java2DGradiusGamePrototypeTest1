 ...
 * @author admin
 */
public class PowerUpBlue extends Entity {
    
    public double vx;
    
    public PowerUpBlue(View view, int x, int y) {
        super(view);
        this.x = x;
        this.y = y;
        vx = -1;
    ...
    @Override
    public void start() {
        loadFrame("power_up_blue_0");
        loadFrame("power_up_blue_1");
        loadFrame("power_up_blue_2");
    }
    ...
    public void update() {
        ...
        if (CollisionBuffer.get(collisionArea.x, collisionArea.y, CollisionBuffer.SHIP)
            ...
            Audio.playSound("power_up_blue_pick");
            view.explodeAllEntities();
            destroy();
        }        
